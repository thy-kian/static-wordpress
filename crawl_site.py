#!/usr/bin/env python3
"""
Static site crawler - mirrors nalamiz.gamenic.io locally
...
BASE_URL = "https://nalamiz.gamenic.io"
OUTPUT_DIR = Path("c:/Users/Administrator/Downloads/Writerity/nalamiz.gamenic.io")
HEADERS = {
...
def safe_asset_path(url: str) -> Path:
    ...
    # Domain prefix for external assets
    is_external = parsed.netloc and parsed.netloc not in (
        "nalamiz.gamenic.io", "www.nalamiz.gamenic.io", ""
    )
    if is_external:
        domain_slug = make_safe_slug(parsed.netloc.replace("www.", "").split(".")[0], max_len=15)
...
def safe_page_path(url: str) -> Path:
    ...
    if parsed.query:
        qhash = hashlib.md5(parsed.query.encode()).hexdigest()[:8]
        raw_path = (raw_path.rstrip("/") + "_" + qhash) if raw_path else qhash
    if not raw_path:
...
def is_same_domain(url: str) -> bool:
    parsed = urllib.parse.urlparse(url)
    return parsed.netloc in ("", "nalamiz.gamenic.io", "www.nalamiz.gamenic.io")
